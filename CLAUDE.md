...
- **Data Versions**: v1 databases (`benign.db`, `campaigns.db`) contain log events only. v2 databases (`benign_v2.db`, `campaigns_v2.db`) added eBPF kernel events. **v3 database** (`benign_v3.db`) is a clean rebuild from personal server logs (no hospital PII) — built via `scripts/build_benign_v3.py` which accepts any server log tarballs containing auth.log, syslog, nginx, or apache2 logs. Malicious traffic (SSH brute force, scanner probes, SQLi, XSS, JNDI) is automatically filtered; the `MaliciousFilter` also accumulates attacker IPs from filtered auth/web events and uses them to filter `ebpf_network` events during eBPF carryover (Phase 9c). PII (hostnames, domains, IPs) is scrubbed to match the campaign target server (`isildur` / `192.168.2.201`) so all benign + attack data appears to come from one host. Scrubbing is configurable via JSON config (`--scrub-config`) or disabled with `--no-scrub`. Multiple `--ebpf-source` DBs can be specified (one per server). Build report at `data/build_benign_report.json` (includes `ebpf_events_filtered` count). Composition configs and experiment streams now use `benign_v3.db`.
- **eBPF Baseline Collections** (2026-03-22): 24-hour collections from 3 Debian 13.4 servers — frodo (2,355,832 events, `data/ebpf_frodo.db`), 9600baud (785,473 events, `data/ebpf_9600baud.db`), hopper (102,078 events, `data/ebpf_hopper.db`). Total: 3,243,383 benign eBPF events. Pending rebuild of benign_v3.db with `--ebpf-source` and recomposition.
...
python scripts/build_benign_v3.py --source web1:web1.tar --ebpf-source data/ebpf_s1.db --ebpf-source data/ebpf_s2.db --output data/benign.db  # Multiple eBPF sources
