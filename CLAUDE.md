...
- **v4 Experiment Streams** (composed from benign_v4.db + campaigns_v2.db, 2026-03-22): exp_7d_brute_v4.db (140K events, 6 campaigns, 101MB), exp_30d_heavy_v4.db (1.1M events, 284 campaigns, 692MB), exp01_90d_v4.db (2.0M events, 277 campaigns, 1.4GB), exp_365d_realistic_v4.db (9.6M events, 897 campaigns, 6.6GB). All 4 validated: temporal order PASS, label consistency PASS, session coherence PASS. Composition must run sequentially on systems with ≤24GB RAM — parallel compose OOMs because benign_v4.db (5.8GB) is loaded into memory per process.
