...
- **Environment (v2, planned)**: `SecurityLogStream-v2` — hybrid observation space reflecting real SOC tooling. Text channels for human-readable logs (auth_log, syslog, web_log); structured `Box` channels for eBPF kernel events (process_events, network_events, file_events as fixed-width numeric arrays with xxhash string hashing); new `lsm_events` channel for BPF LSM security decision events. See ROADMAP.md Phase 13 for full design.
